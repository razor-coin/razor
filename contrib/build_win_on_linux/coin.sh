...
HEADERS = $(wildcard *.h) " | sed s2"STAGING_HERE"2"$(echo $STAGING)"2g | patch -lmake -f makefile.linux-mingw $MAKEOPTS DEPSDIR=$STAGING razord.exe USE_UPNP=0 DEBUGFLAGS="-frandom-seed=razor" USE_SSE2=1
$HOST-strip *.exe
rm -rf $OUTDIR/daemon
mkdir $OUTDIR/daemon
