...
void ChatWindow::connecte()
{
    ...
    ui->tab->setTabToolTip(ui->tab->count()-1,"irc.altcoinweb.com");
    // current tab is now the last, therefore remove all but the last
    for (int i = ui->tab->count(); i > 1; --i) {
       ui->tab->removeTab(0);
    }

    serveurs.insert("irc.altcoinweb.com",serveur);

	serveur->pseudo=ui->editPseudo->text();
    serveur->serveur="irc.altcoinweb.com";
    serveur->port=6667;
    ...
    serveur->connectToHost("irc.altcoinweb.com",6667);
